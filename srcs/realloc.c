...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/22 15:12:08 by cmalfroy          #+#    #+#             */
/*   Updated: 2018/08/23 20:10:46 by cmalfroy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			*realloc(void *ptr, size_t size)
{
	...
	if ((b = find_ptr(get_mem()->tiny, ptr)) != NULL ||
		(b = find_ptr(get_mem()->small, ptr)) != NULL)
	{
		if (size == b->size)
		{
			pthread_mutex_unlock(get_lock());
	...
	else if ((b = find_ptr(get_mem()->large, ptr)) != NULL)
