...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/22 15:12:39 by cmalfroy          #+#    #+#             */
/*   Updated: 2018/08/23 23:04:13 by cmalfroy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		free(void *ptr)
{
	...
	if (try_free(get_mem()->tiny, ptr, \
		(void*)get_mem()->tiny + TINY_MIN) == 1)
		;
	else if (try_free(get_mem()->small, ptr, \
		(void*)get_mem()->small + SMALL_MIN) == 1)
		;
	else
		try_free_large(get_mem()->large, ptr);
